'''
Utility script to flatten the REACH fish short term studies to a single excel file.
...
The output of this script is fish_acute/processed/REACH_short_term_fish_raw_validation.xlsx
'''
# setup logging
import logger
import logging
...
docs = pd.read_parquet(r'data/fish_acute/raw/ENDPOINT_STUDY_RECORD_ShortTermToxicityToFish_esr.parquet')
# keep only the rows with endpoint study records
msk = docs['UUID (endpoint study record) (echachem)'].notnull()
docs = docs.loc[msk]
log.info(f'number of endpoint study records for in vivo genotoxiciy: {len(docs)}')
...
for idx, row in tqdm(docs.iterrows(), 'processing endpoint study records'):
    ...
    endpoint_study_record['principles of method if other than guideline'] = esr_dict.get('MaterialsAndMethods', dict()).get('MethodNoGuideline')
    # GLP compliance
    ...
    # water media type
    endpoint_study_record['water media type'] = esr_dict.get('MaterialsAndMethods', dict()).get('StudyDesign', dict()).get('WaterMediaType')
    # limit test
    ...
    # hardness
    endpoint_study_record['hardness'] = esr_dict.get('MaterialsAndMethods', dict()).get('TestConditions',dict()).get('Hardness')
    # test temperature
    ...
    # salinity
    endpoint_study_record['salinity'] = esr_dict.get('MaterialsAndMethods', dict()).get('TestConditions', dict()).get('Salinity')
    # conductivity
    endpoint_study_record['conductivity'] = esr_dict.get('MaterialsAndMethods', dict()).get('TestConditions', dict()).get('Conductivity')
    ...
    endpoint_study_record['high dose level used'] = esr_dict.get('MaterialsAndMethods', dict()).get('TestConditions', dict()).get('HighDoseLevelUsed')
    # any other information on materials and methods incl tables (always empty?)
    endpoint_study_record['any other information on materials and methods incl tables'] = (esr_dict
                                                                                           .get('MaterialsAndMethods', dict())
                                                                                           .get('AnyOtherInformationOnMaterialsAndMethodsInclTables', dict())
                                                                                           .get('OtherInformation'))
    if pd.notnull(endpoint_study_record['any other information on materials and methods incl tables']):
        soup = BeautifulSoup(endpoint_study_record['any other information on materials and methods incl tables'], 'html.parser')
        endpoint_study_record['any other information on materials and methods incl tables'] = soup.get_text()
...
# guidelines seen in the data
# endpoint_study_records['guideline'].explode().apply(lambda g: g.get('Guideline', dict()) if pd.notnull(g) else None).dropna().apply(lambda g: g.get('code (mapped)')).value_counts()
# OECD guidelines seen in other
# other_guidelines = endpoint_study_records['guideline'].explode().apply(lambda g: g.get('Guideline', dict()) if pd.notnull(g) else None).dropna().apply(lambda g: g.get('other'))
# other_guidelines = other_guidelines[other_guidelines.str.contains(r'(?i)(?:OECD|TG)\s*\d{3}', na=False, regex=True)]
# other_guidelines.to_excel('junk/2025_01_15_other_acute_guidelines.xlsx', index=False)


guideline_tagging = [
    ('short-term toxicity to fish', ['LC50'],        'OECD Guideline 203',  r'(?i)(?:OECD|TG|Guideline)[^\d]*203'),
    ('short-term toxicity to fish', ['LC50'],        'EU Method C.1',          r'(?i)EU[^\d]*C[\.\s\-]*1'),
    ('short-term toxicity to fish', ['LC50'],        'EPA OPPTS 850.1075', r'(?i)(?:EPA|OPPTS|TSCA)[^\d]*850[\.\s\-]+1075'),
    ('short-term toxicity to fish', ['LC50'],        'EPA OPP 72-1',           r'(?i)(?:EPA|OPP|TSCA)[^\d]*72[\.\s\-]+1'),
    ('short-term toxicity to fish', ['LC50'],        'EPA OTS 797.1400',     r'(?i)(?:EPA|OTS|TSCA)[^\d]*797[\.\s\-]+1400'),
    ('short-term toxicity to fish', ['LC50'],        'ISO 7346-1',                r'(?i)ISO[^\d]*7346[\.\s\-/]+1'),

    ('fish embryo acute toxicity test',     ['LC50'],   'OECD Guideline 236', r'(?i)(?:OECD|TG|Guideline)[^\d]*236'),
    ('fish embryo acute toxicity test',     ['LC50'],   'EU Method C.49', r'(?i)EU[^\d]*C[\.\s\-]*49'),

    ('fish cell line acute toxicity in vitro', ['EC50'], 'OECD Guideline 249', r'(?i)(?:OECD|TG|Guideline)[^\d]*249'),
]
...
endpoint_study_records.to_pickle(rf'data/fish_acute/processed/REACH_short_term_fish_raw_validation.pickle')

# export the raw dataset for validation
...
endpoint_study_records.to_excel(rf'data/fish_acute/processed/REACH_short_term_fish_raw_validation.xlsx', index=False)
