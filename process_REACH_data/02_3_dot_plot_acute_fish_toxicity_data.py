...
import logger
import logging
log = logger.setup_applevel_logger(file_name ='logs/02_3_dot_plot_acute_fish_toxicity_data.log', level_stream=logging.INFO, level_file=logging.DEBUG)
...
msk = datasets['scenario'].str.split('_').str[1].isin(['1a', '1b', '2a', '2b', '3a', '3b', '4a', '4b'])
tmp = datasets.loc[msk]
AFT_scenarios = tmp.query('scenario.str.startswith("AFT")', engine='python')['scenario'].dropna().drop_duplicates().sort_values().to_list()
FET_scenarios = tmp.query('scenario.str.startswith("FET")', engine='python')['scenario'].dropna().drop_duplicates().sort_values().to_list()
FCL_scenarios = tmp.query('scenario.str.startswith("FCL")', engine='python')['scenario'].dropna().drop_duplicates().sort_values().to_list()
data = tmp.assign(dummy=1).pivot_table(index='dtxsid', columns='scenario', values='dummy', aggfunc='sum').map(pd.notnull).groupby(AFT_scenarios+FET_scenarios+FCL_scenarios).size()
fig = plt.figure()
...
for scenario in AFT_scenarios:
    upset.style_subsets(present=scenario, absent=FET_scenarios+FCL_scenarios, facecolor="black", label='AFT')
for scenario in FET_scenarios:
    upset.style_subsets(present=scenario, absent=AFT_scenarios+FCL_scenarios, facecolor="orange", label='FET')
for scenario in FCL_scenarios:
    upset.style_subsets(present=scenario, absent=AFT_scenarios+FET_scenarios, facecolor="olive", label='FCL')
...
fig.savefig(rf'figures/REACH_short_term_fish_toxicity_LC50_upset_plot_scenario_a_b.png', dpi=600)
...
for label, scenario_suffix, zorder, color in zip(['scenarios 1a, 1b', 'scenarios 2a, 2b, 3a, 3b, 4a, 4b'], [['1a', '1b'],['2a','2b','3a','3b','4a','4b']], [2, 1], ['black', 'orange']):
...
fig.savefig(rf'figures/REACH_short_term_fish_toxicity_LC50_dot_plot.png', dpi=600)
